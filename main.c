...
void main_menu(void) {
    while (1) {
        ...
        show_deck_list();
        puts("  1. Study a deck");
        puts("  2. Create a new deck");
        puts("  3. Delete a deck");
        puts("  4. View stats");
        puts("  0. Quit");
        printf("  > ");
        ...
        if (ch == 0) {
            ...
            printf("  Delete deck \"%s\" and all its cards? [y/N]: ",g.decks[idx].name);
            char yn[8]; read_line(yn, sizeof(yn));
            if (yn[0] != 'y' && yn[0] != 'Y') continue;
